<template>
   <modal>
    <div class=" md:w-2/3 lg:w-1/2 xl:w-5/12 mx-4 md:mx-auto bg-white rounded-2xl p-10 lg:py-16 lg:px-24 relative text-primary">
      ...
      <div>
        <form action="" class="grid grid-rows-2 gap-y-11 pt-11">
          ...
          <div>
            <button class="bg-grid1 hover:bg-secondary text-white rounded-15px py-3 w-full font-bold">
              <router-link to="/profile">SEND RESET TOKEN</router-link>
            </button>
          </div>
        </form>

        <div class="py-6">
          <router-link to="/password-reset" class="underline text-secondary">I have a reset token.</router-link>
        </div>
